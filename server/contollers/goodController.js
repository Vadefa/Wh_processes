...
class GoodController{
    async create(req, res, next){
        try{
            ...
            //7. Находим оптимальный маршрут
            //7.1 Строим матрицу расстояний
            //let opt = b_b.main()
            let temp_rack;
            let temp_model;
            let a_models = new Array();
            let a_racks = new Array();
            let way = '';
            try{
                for(let i = 0; i < arr_shelves_of_new_goods.length; i++){
                    temp_model = await Model.findOne({
                            where: {id: arr_models_id[i]}
                        })
                    a_models.push(temp_model.name);
                    for (let j = 0; j < arr_shelves_of_new_goods[i].length; j++){
                        temp_rack = await Shelf.findOne({
                            where: {id: arr_shelves_of_new_goods[i][j]
                            },
                            attributes: ['rackId']
                        })
                        a_racks.push(parseInt(temp_rack.rackId));
                   }
                }
                if (a_racks.length > 50){
                    way = 'Число добавляемых товаров превышает 50, поэтому поиск маршрута не будет выполнен.';
                }
                else{
                    let m_shelves = new Array()
                    for(let i = 0; i < arr_shelves_of_new_goods.length; i++){
                        let init_sh = arr_shelves_of_new_goods[i][0];
                        let sh = arr_shelves_of_new_goods[i][0];
                        let m_count = 0;
                        let i_shift = i == 0 ? 0 : arr_shelves_of_new_goods[i - 1].length;
                        for(let j = 0; j < arr_shelves_of_new_goods[i].length; j++){
                            if (arr_shelves_of_new_goods[i][j] == sh)
                                m_count++;
                            else{
                                m_shelves.push([a_models[i], a_racks[i_shift + j], parseInt(sh), m_count])
                                sh = arr_shelves_of_new_goods[i][j];
                                m_count = 1;
                            }
                        }
                        if (init_sh != sh){
                            m_shelves.push([a_models[i], a_racks[i_shift + arr_shelves_of_new_goods[i].length - 1], parseInt(sh), m_count])
                        }
                    }
                    console.log("arr_shelves_of_new_goods");
                    console.log(arr_shelves_of_new_goods)
                    ...
                    if (a_racks.length > 0 && a_racks.length < 60){
                    ...
                    if (a_racks.length == 1){
                        ...
                        console.log("Число мест для посещения:" + pr_matr.length.toString())
    ...
    async deleteByModelId(req, res, next){
        ...
        try{
            ...
            const del_rack = await Shelf.findOne({
                where: {id: shelf_id.shelfId
                },
                attributes: ['rackId']
            })

            let resp = 'OK ' + shelf_id.shelfId.toString() + ' ' + del_rack.rackId.toString()

            console.log(resp)
            console.log(resp.split(' ')[0])
            //return res.json({message: "OK"}/*, shelf_id.shelfId.toString()*/)
            return res.json({message: "OK " + shelf_id.shelfId.toString() + " " + del_rack.rackId.toString()})
            //return res.json(resp)
        } catch(e){
            return next(ApiError.badRequest("вытаскиваем номер полки: " + e.message))
        }
    }
}
